...
from environments import TURNSTILE_SUMMARY_TOPIC,TURNSTILE_TOPIC_NAME,TRNAFORMED_STATION_TOPIC_NAME,STATION_TOPIC_PREFIX,TURNSTILE_TOPIC_NAME
...
class Line:
    ...
    def process_message(self, message):
        ...
        if topic_name ==TRNAFORMED_STATION_TOPIC_NAME: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif STATION_TOPIC_PREFIX in topic_name: # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif topic_name ==TURNSTILE_SUMMARY_TOPIC: # Set the conditional to the KSQL Turnstile Summary Topic
