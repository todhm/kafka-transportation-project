...
from environments import NUM_PARTITIONS_PER_TOPIC,NUM_REPLICAS_PER_TOPIC,STATION_TOPIC_PREFIX
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        try:
            self.producer.produce(
                ...
                value=self.to_dict(train,direction,prev_station_id,prev_direction),
            )
            logger.debug("Station produce success")
        except Exception as e :
            logger.error(f"error produce station data {self.topic_name}"+str(e))
